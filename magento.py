...
def test_eh_sign_up_error_notification(setup, users):
    ...
    # check that in the absence of '@' in the mail, a corresponding message appears
    if "@" not in users[2]:
    ...
    # check that if there is no name, a corresponding message appears
    ...
    # check that in the absence of a surname, a corresponding message appears
    if not users[1].strip():
        error_message = driver.find_element(By.ID, "lastname-error").text
        assert error_message == "This is a required field."

    # check that if '.com' is missing, a corresponding message appears
...
def test_fn_add_to_wish_list(setup):
    ...
    '//ol[@class="product-items widget-product-grid"]/li[4]//a[contains(.,"Add to Wish List")]')
...
def test_fn_remove_from_wish_list(setup):
    ...
    menu_click = driver.find_element(By.XPATH, '//div[@class="panel header"]//button[@class="action switch"]')
    menu_click.click()
    time.sleep(3)
...
def test_fn_add_to_comparison_list(setup):
    ...
    driver.find_element(By.XPATH, '//a[text()="Hoodies and Sweatshirts"]').click()
    time.sleep(3)
...
def test_edit_cart_qty(setup):
    ...
    click_on_cart = driver.find_element(By.XPATH, '/html/body/div[2]/header/div[2]/div[1]/a')
    click_on_cart.click()
    time.sleep(3)
